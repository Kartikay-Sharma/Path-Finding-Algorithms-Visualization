...
Description:
    This program is a visualization tool for pathfinding algorithms, enabling
    users to search a path between start and end cells on a grid.
    The user can add wall/obstacle on the grid and the algorithms will find a new path
    between start and end while considering the walls.
    Additionally, a random maze generator feature is integrated, which creates
    random mazes on the grid.
...
pygame.init()
pygame.display.set_caption("Path Finding Algorithms Visualization Tool")
